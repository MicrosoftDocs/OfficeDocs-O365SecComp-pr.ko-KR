...
|+|keyword1 + keyword2 + keyword3|`keyword3` ** `keyword1` ** 또는가 포함 된 항목을 반환 합니다.`keyword2`   따라서이 예제는 쿼리와 `(keyword2 OR keyword3) AND keyword1`동일 합니다.  <br/> 기호 뒤 `keyword1 + keyword2` 에 공백이 있는 쿼리는 * * 및 * * 연산자를 사용 하는 것과는 다릅니다. **+** 이 쿼리는 해당 하는 `"keyword1 + keyword2"` 것과 동일한 작업을 포함 하 `"keyword1 + keyword2"`는 항목을 반환 합니다.|
