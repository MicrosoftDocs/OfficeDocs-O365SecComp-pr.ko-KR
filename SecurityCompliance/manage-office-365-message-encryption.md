 ...
 **OME 포털에 로그인 하려면 공유 Id를 사용 하 여 받는 사람에 게 허용 여부를 관리 하려면**
 ...
 **1 회 암호가 OME에 대해 생성 되는 여부를 관리 하려면**
  
1. [원격 PowerShell을 사용 하 여 온라인 Exchange에 연결](http://technet.microsoft.com/library/jj984289%28v=exchg.150%29.aspx)합니다.
    
2. 다음과 같이 OTPEnabled 매개 변수와 함께 Set-omeconfiguration cmdlet를 실행 합니다.
 ...
 **관리 여부는 Protect 단추 표시 하는 Outlook에서 웹에**
  
1. [원격 PowerShell을 사용 하 여 온라인 Exchange에 연결](http://technet.microsoft.com/library/jj984289%28v=exchg.150%29.aspx)합니다.
    
2. 다음과 같이-SimplifiedClientAccessEnabled 매개 변수와 함께 Set-irmconfiguration cmdlet를 실행 합니다.
 ...
 **IOS 메일 앱 사용자 여부를 관리 하려면 Office 365 메시지 암호화로 보호 된 메시지를 볼 수 있습니다.**
  
1. [원격 PowerShell을 사용 하 여 온라인 Exchange에 연결](http://technet.microsoft.com/library/jj984289%28v=exchg.150%29.aspx)합니다.
    
2. 다음과 같이 AllowRMSSupportForUnenlightenedApps 매개 변수와 함께 집합 ActiveSyncOrganizations cmdlet를 실행 합니다.
  ...
  ```
    ...
    예: unenlightened 응용 프로그램을 암호가 해독 된 메시지를 보내지 서비스를 구성 하려면:
...
기본 인 암호가 해독 된 전자 메일 첨부 파일을 허용 하지 않도록 선택 하는 경우 사용자가 첨부 파일을 볼 수 있는 권한이 없더라도 없다는 메시지를 수신 합니다. \* \* \* 그림을 삽입?
 ...
 **관리 여부 전자 메일 첨부 파일의 암호 해독에 웹 브라우저에서 다운로드**
  
1. [원격 PowerShell을 사용 하 여 온라인 Exchange에 연결](http://technet.microsoft.com/library/jj984289%28v=exchg.150%29.aspx)합니다.
    
2. 다음과 같이 DecryptAttachmentFromPortal 매개 변수와 함께 Set-irmconfiguration cmdlet를 실행 합니다.
 ...
 **OME에 대 한 새로운 기능을 사용 하지 않도록 설정 하려면**
  
1. [원격 PowerShell을 사용 하 여 온라인 Exchange에 연결](https://technet.microsoft.com/library/jj984289%28v=exchg.160%29.aspx)합니다.
    
2. 웹에 있는 Outlook에서 암호로 보호 단추를 사용 하도록 설정한 경우 다음과 같이 SimplifiedClientAccessEnabled 매개 변수와 함께 Set-irmconfiguration cmdlet을 실행 하 여 해제 합니다.
    
  ```
  Set-IRMConfiguration -SimplifiedClientAccessEnabled $false
  ```

3. 다음과 같이 AzureRMSLicensingEnabled 매개 변수와 함께 Set-irmconfiguration cmdlet를 실행 합니다.
