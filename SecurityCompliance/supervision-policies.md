...
비속어를 위해 조직의 전자 메일 메시지를 보내거나 받게 됩니다. 모델에서는 기계 학습, 인공 지능 및 키워드를 함께 사용 하 여 harassment 및 사이버 문서 모니터링 요구 사항의 일부로 적합 하지 않은 전자 메일 메시지를 식별 합니다. 조직의 다른 통신에 대 한 원하지 않는 언어를 방지 하거나 차단 하려면 공격적인 용어의 [사용자 지정 키워드 사전을](create-a-keyword-dictionary.md) 사용 하는 [데이터 손실 방지 정책을](create-test-tune-dlp-policy.md) 만듭니다.
...
여러 조건을 입력 하면 Office 365에서 모든 조건을 함께 사용 하 여 통신 항목에 정책을 적용할 시기를 결정 합니다. 여러 조건을 설정 하는 경우 예외를 입력 하지 않으면 정책이 적용 되려면 모두 충족 되어야 합니다. 예를 들어 메시지에 "무역" 이라는 단어가 들어 있고 2mb 보다 큰 경우에 적용 되는 정책을 만들어야 합니다. 그러나 메시지에 "Contoso 재무에서 승인" 이라는 단어가 포함 된 경우에는 정책이 적용 되지 않아야 합니다. 따라서이 경우 세 가지 조건은 다음과 같습니다.
...
## <a name="monitor--manage"></a>_AMP_ 관리 모니터링

감독 정책의 결과를 쉽게 모니터링 하 고 해결 태그를 적용할 수 있습니다. 검토 한 항목의 상태, 감독 아래에 있는 사용자 및 그룹, 검토자에 게 지정 된 사용자 및 그룹을 빠르게 확인할 수 있습니다.

### <a name="supervision-policy-dashboard"></a>감독 정책 대시보드

감독 정책 대시보드를 사용 하 여 감독 정책 결과를 관리 하 고 처리 중인 항목을 해결할 수 있습니다. 이 대시보드를 사용 하면 검토자가 검토 해야 하는 항목을 보고, 항목에 대해 작업을 수행 하 고, 각 감독 정책에 대해 이전에 검토 및 확인 한 항목의 결과를 검토할 수 있습니다. 규정 **** > 준수 센터에서*사용자 지정 정책* > **공개**에 대해 감독 정책 대시보드에 액세스할 수 있습니다.
...
2. **보고서** \> **대시보드** 또는 **감독** 에 게 이동 하 여 현재 감독 정책 활동을 요약해 서 감독 보고 위젯을 확인 하세요.
