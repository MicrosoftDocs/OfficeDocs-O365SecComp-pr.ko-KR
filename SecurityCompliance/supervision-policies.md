...
보안 & 준수 센터에서 감독 정책을 만듭니다. 이러한 정책은 조직의 검토 대상이 되는 통신 및 사용자를 정의 하 고, 통신이 충족 해야 하는 사용자 지정 조건을 정의 하 고, 검토를 수행할 사람을 지정 합니다. 관리 검토 역할 그룹에 포함 된 사용자는 정책을 설정할 수 있으며이 역할에 할당 된 모든 사용자는 Office 365 보안 & 준수 센터의 데이터 거 버 넌 스에 있는 감독 페이지에 액세스할 수 있습니다.
...
감독 정책을 만들 때 감독 된 사용자의 메시지 검토를 수행할 사용자도 결정 됩니다. 정책에서는 사용자 전자 메일 주소를 사용 하 여 감독 된 통신을 검토할 개인 또는 사용자 그룹을 식별 합니다.
...
여러 조건을 입력 하면 Office 365에서 모든 조건을 함께 사용 하 여 통신 항목에 정책을 적용할 시기를 결정 합니다. 여러 조건을 설정 하는 경우 예외를 입력 하지 않으면 정책이 적용 되려면 모두 충족 되어야 합니다. 예를 들어 메시지에 "무역" 이라는 단어가 포함 되어 있고 크기가 2mb 보다 크면 적용 되는 정책을 만들어야 한다고 가정해 보겠습니다. 그러나 메시지에 "Contoso 재무에서 승인" 이라는 단어가 포함 된 경우에는 정책이 적용 되지 않아야 합니다. 따라서이 경우 세 가지 조건은 다음과 같습니다.
...
검토할 콘텐츠의 양을 줄이려면 감독 정책으로 제어 되는 모든 통신의 백분율을 지정할 수 있습니다. 선택한 조건에 맞는 총 백분율에서 해당 콘텐츠 양을 임의로 선택 합니다. 검토자가 모든 항목을 검토 하도록 하려면 감독 정책에 **100%** 를 입력 하면 됩니다.
...
감독 정책 결과를 관리 하 고 처리 중인 항목을 해결 하는 가장 쉬운 방법은 감독 정책 대시보드를 사용 하는 것입니다. 이 대시보드를 사용 하면 검토자가 검토 해야 하는 항목을 빠르게 확인 하 고, 항목에 대해 작업을 수행 하 고, 각 감독 정책에 대해 이전에 검토 및 확인 된 항목의 결과를 검토할 수 있습니다. **** > *사용자 지정 정책을* > **열면**Office 365 Security & 준수 센터의 감독 정책 대시보드에 액세스할 수 있습니다.
...
감독 정책에서 검토자로 지정 된 사용자는 웹에서 Outlook을 사용 하 여 감독 항목을 검토 하 고 확인할 수 있습니다. 감독 추가 기능은 정책에 지정 된 모든 검토자에 대해 웹용 Outlook에 자동으로 설치 됩니다. 조직에서 구성 된 검토자에 게 감독 정책 공유 폴더를 사용할 수 있도록 하는 추가 구성은 필요 하지 않습니다.

웹에서 Outlook을 사용 하 여 다음과 같은 작업을 수행할 수 있습니다.
...
Outlook을 사용 하 여 다음과 같은 작업을 수행할 수 있습니다.
...
1. 감독 보고서를 볼 수 있는 권한이 있는 Office 365 조직의 관리자 계정에 대 한 자격 증명을 사용 하 여 [보안 & 준수 센터](https://protection.office.com/) 에 로그인 합니다.
...
다음과 같은 감독 정책 활동을 감사 하 고 통합 Office 365 감사 로그를 사용 하 여 볼 수 있습니다.
