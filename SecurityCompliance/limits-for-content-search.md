...
|콘텐츠 검색에 대 한 검색 쿼리 (연산자 및 조건 포함)에 대 한 문자의 최대 수입니다.  <br/> **참고:** 이 제한은 각 키워드에 대해 쿼리를 확장 얻을 것을 의미 하는 쿼리를 확장 한 후에 적용이 됩니다. 등 검색 쿼리를 15 키워드와 추가 매개 변수 및 조건 있으면 쿼리 가져옵니다 확장 15 번 각각 다른 매개 변수 및 쿼리의 조건입니다. 검색 쿼리의 문자 수 제한 아래 수, 경우에 이므로 확장 된 쿼리를이 제한을 초과 하면 현상이 발생할 수 있습니다.<br/> |**사서함:** 10, 000 개  <br/> **사이트:** 4000 최대 20 개의 사이트 <sup>1</sup> 을 검색 하는 경우 모든 사이트 또는 2, 000를 검색 하는 경우 <br/> |
...
|**인덱싱 제한**|**참고**|**설명**|
|:-----|:-----|:-----|
|최대 첨부 파일 크기 (Excel 파일은 제외)  <br/> |150MB  <br/> |인덱싱에 대 한 구문 분석 하는 전자 메일 첨부 파일의 최대 크기입니다. 인덱싱에 대해이 제한 보다 큰 첨부 파일을 구문 분석할 수 없습니다 하 고 부분적으로 인덱싱되는 첨부 파일이 있는 메시지에 표시 됩니다.<br/> > [!NOTE]> 구문 분석 여기에서 인덱싱 서비스 텍스트에서에서 추출 첨부 파일, 문장 부호 및 공백와 같은 불필요 한 문자를 제거 하 고 다음 인덱스에 저장 되는 텍스트 (이라는 프로세스에서 토큰화), 단어에이 값을 사용 하는 프로세스입니다.           |
