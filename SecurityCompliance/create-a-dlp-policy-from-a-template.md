...
DLP 정책을 시작 하는 가장 쉽고 일반적인 방법은 Office 365에 포함 된 서식 파일 중 하나를 사용 하는 것입니다. 이러한 서식 파일 중 하나를 그대로 사용 하거나 조직의 특정 규정 준수 요구 사항을 충족 하도록 규칙을 사용자 지정할 수 있습니다.
...
비즈니스용 OneDrive 계정을 통해 조직의 사용자가 쉽게 문서를 공동 작업 하 고 공유할 수 있습니다. 그러나 규정 준수 관리자는 비즈니스용 OneDrive 계정에 저장 된 중요 한 정보가 실수로 조직 외부의 사용자와 공유 될 수 있다는 것을 고려해 야 합니다. DLP 정책을 통해 이러한 위험을 완화할 수 있습니다.
...
- 사용자가 비즈니스 근거를 제공 하거나 가양성을 보고 하 여 작업을 재정의할 수 있도록 허용 합니다. 이러한 방식으로 중요 한 정보를 공유 하는 유효한 비즈니스 이유가 있는 경우 DLP 정책은 조직의 사용자가 작업을 수행 하는 것을 막지 못합니다.
...
6. DLP 정책으로 보호 하려는 위치를 선택 하려면 다음 중 하나를 수행 합니다.
  ...
  - \> **다음**에 **특정 위치 선택을** 선택 합니다. 이 예에서는이를 선택 합니다.
    ...
    특정 SharePoint 사이트 또는 비즈니스용 OneDrive 계정만 포함 하려면 **상태** 를 설정으로 전환한 다음 **포함** 아래의 링크를 클릭 하 여 특정 사이트 또는 계정을 선택 합니다. 사이트에 정책을 적용 하면 해당 정책에 구성 된 규칙이 해당 사이트의 모든 하위 사이트에 자동으로 적용 됩니다. 
...
8. DLP 정책 템플릿에는 특정 유형의 중요 한 정보를 검색 하 고 처리 하는 조건과 동작이 있는 미리 정의 된 규칙이 포함 되어 있습니다. 기존 규칙을 편집, 삭제 또는 해제 하거나 새로 추가할 수 있습니다. 완료 되 면 **다음**을 클릭 합니다.
...
15. \> **다음**규칙에 대 한 조건 및 작업을 검토 하십시오.
    ...
    오른쪽에서 규칙에 대 한 **Status** 스위치를 확인 합니다. 전체 정책을 해제 하면 해당 정책에 포함 된 모든 규칙도 꺼집니다. 그러나 전체 정책을 해제 하지 않고 특정 규칙을 해제할 수 있습니다. 이는 많은 수의 가양성을 생성 하는 규칙을 조사 해야 하는 경우에 유용 합니다. 
    
16. 다음 페이지에서 다음 사항을 읽고 이해 한 다음 규칙을 켤 지, 아니면 먼저 \> **** 테스트를 수행할지를 선택 합니다.
    
     DLP 정책을 만들기 전에 점차적으로 해당 영향을 평가 하 고 효과를 완벽 하 게 적용 하기 전에 테스트 해야 합니다. 예를 들어 새 DLP 정책이 사용자가 작업을 완료 하는 데 필요한 수천 개의 문서에 대 한 액세스를 차단 하는 것을 원하지 않을 수 있습니다. 
...
19. 규칙이 적용 되 고 콘텐츠가 보호 되도록 정책을 설정 합니다. 계속 해 서 DLP 보고서와 문제 보고서 또는 알림을 모니터링 하 여 결과가 의도 한 대로 진행 되는지 확인 합니다. 
...
언제 든 지 보안 &amp; 및 준수 센터의 **데이터 손실 방지** 섹션에 있는 **정책** 페이지에서 DLP 정책의 상태를 볼 수 있습니다. 여기서 정책이 성공적으로 사용 하거나 사용 하지 않도록 설정 되었는지 여부 또는 정책이 테스트 모드에 있는지와 같은 중요 한 정보를 찾을 수 있습니다. 
