...
DLP (데이터 손실 방지) 정책을 사용 하 여 Office 365에서 중요 한 정보를 식별, 모니터링 및 보호할 수 있습니다. 조직의 사용자가 이러한 중요 한 정보를 사용 하 여 DLP 정책을 준수 하 고 불필요 하 게 작업을 수행 하는 것을 차단 하지 않으려고 합니다. 여기서는 전자 메일 알림 및 정책 팁이 도움이 될 것입니다.
...
2. 회사 또는 학교 계정을 사용 하 여 Office 365에 로그인 합니다. 현재는 Office 365 보안 &amp; 및 준수 센터에 있습니다.
...
> [!NOTE]
>  전자 메일 알림은 그룹 또는 메일 그룹이 아닌 개별 받는 사람 에게만 보낼 수 있습니다. >는 새 콘텐츠만 전자 메일 알림을 트리거합니다. 기존 콘텐츠를 편집 하면 정책 팁이 트리거되어 전자 메일 알림이 제공 되지 않습니다. 
...
|액세스를 차단하고, 알림을 보내고, 재정의를 허용합니다.  <br/> |이 항목이 조직의 정책과 충돌 합니다. 이 충돌을 해결 하지 않으면이 파일에 대 한 액세스가 차단 될 수 있습니다.  <br/> |전자 메일 메시지가 조직의 정책과 충돌 합니다. 메시지가 모든 받는 사람에 게 배달 되지 않았습니다.  <br/> |
...
- 사용자에 게 해당 콘텐츠가 DLP 정책과 충돌 하 여 충돌을 해결 하기 위한 조치를 취할 수 있도록 알려야 합니다. 기본 텍스트 (아래 표 참조)를 사용 하거나 조직의 특정 정책에 대 한 사용자 지정 텍스트를 입력할 수 있습니다.
    
- DLP 정책을 재정의할 수 있도록 허용합니다. 필요에 따라 다음을 수행할 수 있습니다.
    
  - 사용자가 정책을 재정의 하기 위해 비즈니스 사유를 입력 하도록 요구 합니다. 이 정보는 기록 되며 보안 &amp; 및 준수 센터의 **보고서** 섹션에 있는 DLP 보고서에서 볼 수 있습니다. 
...
2. 두 번째 규칙:이 중요 한 정보의 인스턴스가 5 개 이상 문서에서 검색 되 고 문서가 조직 내의 사용자와 공유 되는 경우 콘텐츠에 대 한 **액세스 차단** 작업에서는 파일에 대 한 사용 권한을 제한 하 고 ** 알림 보내기** 작업을 통해 비즈니스 사유를 제공 하 여이 규칙의 작업을 다시 정의할 수 있습니다. 조직의 비즈니스에서 내부 사용자가 PII 데이터를 공유 해야 하 고, DLP 정책이이 작업을 차단 하지 않도록 해야 하는 경우가 있습니다. 
...
- 콘텐츠가 DLP 정책의 여러 규칙과 일치 하는 것이 가능 하지만 가장 제한적인 최상위 규칙의 정책 팁만 표시 됩니다. 예를 들어 콘텐츠에 대 한 액세스를 차단 하는 규칙의 정책 팁은 알림을 단순히 전송 하는 규칙에서 정책 팁 위에 표시 됩니다. 이를 통해 사용자는 정책 팁의 캐스케이딩을 볼 수 없습니다.
...
|액세스를 차단하고, 알림을 보내고, 재정의를 허용합니다.  <br/> |이 항목이 조직의 정책과 충돌 합니다. 이 충돌을 해결 하지 않으면이 파일에 대 한 액세스가 차단 될 수 있습니다.  <br/> |
...
현재는 추가 조건에 대 한 정책 팁을 표시 하는 작업을 수행 하 고 있습니다. 여기에는 다음이 포함 됩니다.
...
exchange 관리 센터에서 정책 팁을 구성한 경우에는 exchange의 팁을 해제할 때까지 Office 365 보안 &amp; 준수 센터에서 구성한 정책 팁이 웹 및 outlook 2013 이상 outlook의 사용자에 게 표시 되지 않습니다. 관리 센터 이렇게 하면 Office 365 보안 &amp; 및 준수 센터로 전환 하도록 선택할 때까지 현재 Exchange 전송 규칙이 계속 작동 합니다.
...
|**DLP 정책 규칙이 수행하는 작업...**|**기본 정책 팁에 표시되는 내용...**|
|:-----|:-----|
|알림을 보내지만 재정의를 허용 하지 않습니다.  <br/> |이 파일이 조직의 정책과 충돌 합니다. 자세한 내용을 보려면 **파일** 메뉴로 이동 합니다.<br/> |
|액세스를 차단하고, 알림을 보내고, 재정의를 허용합니다.  <br/> |이 파일이 조직의 정책과 충돌 합니다. 이 충돌을 해결 하지 않으면이 파일에 대 한 액세스가 차단 될 수 있습니다. 자세한 내용을 보려면 **파일** 메뉴로 이동 합니다.<br/> |
|액세스를 차단하고 알림을 보냅니다.  <br/> |이 파일이 조직의 정책과 충돌 합니다. 이 충돌을 해결 하지 않으면이 파일에 대 한 액세스가 차단 될 수 있습니다. 자세한 내용을 보려면 **파일** 메뉴로 이동 합니다.<br/> |
