...
|위장 된 사용자  <br/> |보낸 사람이 침입 되 고 되는 사용자 계정입니다.  <br/> **내부** 탭에만 해당 합니다. 이 필드는 단일 전자 메일 주소를 포함 하거나 보낸 사람이 여러 사용자 계정을 스푸핑은, 하는 경우 **둘 이상**포함 합니다.<br/> **외부** 탭에만 해당 합니다. 만 외부 도메인은 보내는 도메인을 포함 하 고 전체 전자 메일 주소를 포함 하지 않습니다.<br/> **팁! 관리자의 고급.** 위장 된 사용자가 보낸 메일 클라이언트에 의해 보낸사람 주소도 표시 되는 주소 이기도 (5322.From) 주소입니다. 이 방법을 header.from 주소를 라고도 합니다. 이 주소의 유효성을 검사 하 여 SPF 체크 되지 않았습니다.           |
...
3. 보안에서 &amp; 준수 센터 **위협 관리** 확장 하 고 \> **정책** \> **스팸 방지**합니다.  
    ...
    ![스팸 방지 페이지에 대 한 액세스를 보여주는 스크린샷](media/0a098e68-5ecf-40d7-984f-d15fcc1f958d.jpg)
  
4. 오른쪽 창에서 **스팸 방지 설정** 페이지에서 **사용자 지정** 탭을 선택 하 고 아래로 스크롤하여 및 **스푸핑 인텔리전스 정책**을 확장 합니다.  
  
    ![스팸 방지 사용자 지정 설정에 대 한 액세스를 보여주는 스크린샷](media/a5112100-0b37-460f-932d-5b2f98157871.jpg)
  
5. 보낸 사람이 도메인 스푸핑의 목록을 보려면 **검토 새 보낸사람** 을 선택 하 고 **사용자 도메인** 탭을 선택 합니다. 
    
    보낸사람을 이미 검토 했을 때 하 고 이전 선택 사항 중 일부를 변경 하려는 경우 선택할 수 있습니다 **표시 나에 게 보낸 I 이미 검토** 대신. 두 경우 모두 다음 창이 표시 됩니다.  
  
    ![위장 된 보낸사람 탭에 대 한 액세스를 보여주는 스크린샷](media/c0c062fd-f4a4-4d78-96f7-2c22009052bb.jpg)
    ...
    ![발신자가 스푸핑할 수 있는지 여부를 보여주는 스크린샷](media/5dbef9cf-103f-49cd-9638-4b0083d6baa7.jpg)
  
6. 변경 내용을 저장 하려면 **저장** 을 선택 합니다. 

Office 365 Enterprise e 5 구독 했거나 추가 기능으로 위협 보호 고급 별도로 구입한 경우에 [스푸핑 인텔리전스 견해](https://docs.microsoft.com/en-us/office365/securitycompliance/walkthrough-spoof-intelligence-insight)를 통해 도메인 스푸핑는 발신자를 관리할 수 있습니다.
    
## <a name="configuring-the-anti-spoofing-policy"></a>스푸핑 방지 정책 구성
<a name="Managespooflist"> </a>

허용 또는 조직에 위장 된 전자 메일을 발송에서 특정 보낸 사람이 차단, 외에도 방법을 엄격 하 게 사용할 수, 필터 및 스푸핑 메시지 발견 되 면 수행할 동작을 구성할 수 있습니다.
...
6. 메시지 도메인간 스푸핑으로 감지 될 때 수행할 작업을 다음으로 구성 합니다. 기본 동작은 메시지 받는 사람의 정크 메일 폴더로 이동 하는 것입니다. 다른 옵션은 격리로 메시지를 보내려고 합니다. 격리로 전송 하는 메시지를 관리 하는 방법에 대 한 자세한 내용은 [Office 365에서 전자 메일 메시지를 격리를](quarantine-email-messages.md)참조 하십시오.  
    ...
    ![스크린샷이 스푸핑 방지 정책 편집 옵션](media/7a868dff-2c4b-46b9-88ca-f2d523ca2307.jpg)
  
7. 사용자가 선택한을 확인 하 고 **저장**을 선택 합니다. 
...
고급 Office 365 관리자의 경우 이러한 검사를 완료할 수 있습니다.
