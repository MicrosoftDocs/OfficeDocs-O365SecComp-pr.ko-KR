...
1. [필수 구성 요소를 검토 합니다.](set-up-atp-anti-phishing-policies.md#phishprereq)
    
2. [ATP 피싱 방지 정책 옵션에 알아보기](set-up-atp-anti-phishing-policies.md#phishpolicyoptions)
    
3. [ATP 피싱 방지 정책 설정](set-up-atp-anti-phishing-policies.md#addphishpolicy)
...
5. 이름, 설명 및 사용자 정책에 대 한 설정을 지정 합니다. 자세한 내용은 [ATP 피싱 방지 정책 옵션에 대 한 설명](set-up-atp-anti-phishing-policies.md#phishpolicyoptions) 을 참조 하십시오. 
...
이 예제에서는 "도메인 및 CEO" 사용자와 가장에서 도메인 보호를 제공 하 고 다음 contoso.com 도메인 내의 사용자에 게 받은 모든 전자 메일에 정책을 적용 하는 이라는 정책을 설정 합니다. 보안 관리자 정책을 이러한 비즈니스 요구를 충족 해야 결정 했습니다.
...
|작업을 선택 합니다.  <br/> |대리 실행된 한 사용자가 전자 메일을 보낼 경우: **다른 전자 메일 주소로 메시지 리디렉션** 선택 하 고 보안 관리자, securityadmin@contoso.com 등의 전자 메일 주소를 입력 합니다.  <br/> 대리 실행된 한 도메인으로 전자 메일을 보낼 경우: **격리 메시지**를 선택 합니다.  <br/> |
...
|에 적용  <br/> |**받는 사람 도메인을**선택 합니다. **다음 중 하나** **선택**을 선택 합니다. **+ 추가**선택 합니다. 목록에서 예: contoso.com 도메인의 이름 옆에 있는 확인란을 선택한 다음 **추가**선택 합니다. **완료**를 선택 합니다.<br/> |
