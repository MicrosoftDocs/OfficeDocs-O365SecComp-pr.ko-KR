...
경고 정책은 경고를 생성할 사용자 또는 관리자 활동, 작업을 수행 하는 경우 경고를 트리거할 사용자 목록, 작업을 수행할 때 활동이 발생 해야 하는 횟수를 정의 하는 임계값 등을 정의 하는 규칙 및 조건 집합으로 구성 됩니다. n 경고가 트리거됩니다. 또한 정책을 분류 하 고 심각도 수준을 할당 합니다. 이러한 두 설정은 보안 및 준수 센터에서 정책을 관리 하 고 알림을 볼 때 이러한 설정을 기준으로 필터링 할 수 있으므로 경고 정책 및 정책 조건이 일치할 때 트리거되는 경고를 관리 하는 데 도움이 됩니다. 예를 들어 동일한 범주의 조건과 일치 하는 경고를 보거나 같은 심각도 수준의 알림을 볼 수 있습니다.
  
경고 정책을 [https://compliance.microsoft.com](https://compliance.microsoft.com) 보고 만들려면로 이동한 후 **경고** \> **경고 정책을**클릭 합니다. 
...
경고를 보려면로 이동한 후 [https://compliance.microsoft.com](https://compliance.microsoft.com) 경고 **** \> **보기**를 클릭 합니다. 
