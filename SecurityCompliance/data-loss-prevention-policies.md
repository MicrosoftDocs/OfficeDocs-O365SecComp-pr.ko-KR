...
비즈니스 표준 및 산업 규정을 준수 하기 위해 조직은 중요 한 정보를 보호 하 고 실수로 인 한 노출을 방지 해야 합니다. 조직 외부에서 누수가 발생 하지 않도록 하려는 중요 한 정보의 예로는 재무 데이터 또는 PII (개인 식별 정보) (예: 신용 카드 번호, 주민 등록 번호, 상태 레코드) 등이 있습니다. office 365 보안 &amp; 및 준수 센터의 DLP (데이터 손실 방지) 정책을 사용 하 여 office 365에서 중요 한 정보를 식별, 모니터링 및 자동으로 보호할 수 있습니다.
...
- **데스크톱 버전의 Excel 2016, PowerPoint 2016 및 Word 2016에서 중요한 정보를 모니터링하고 보호합니다.**
    ...
    Exchange Online, SharePoint online 및 비즈니스용 OneDrive와 마찬가지로 이러한 Office 2016 데스크톱 프로그램은 중요 한 정보를 식별 하 고 DLP 정책을 적용 하는 것과 같은 기능을 포함 합니다. DLP는 사용자가 이러한 Office 2016 프로그램에서 콘텐츠를 공유 하는 경우 지속적인 모니터링을 제공 합니다.
    
- **사용자가 자신의 워크플로를 중단하지 않고 규정 준수 상태를 유지하도록 하는 방법을 알도록 도와줄 수 있습니다.**
    
    DLP 정책에 대 한 사용자를 교육 하 고 작업을 차단 하지 않고 준수 상태를 유지 하는 데 도움이 될 수 있습니다. 예를 들어 사용자가 중요 한 정보가 포함 된 문서를 공유 하려고 할 때 DLP 정책에서 전자 메일 알림을 보내이를 통해 회사에서 정책을 재정의 하도록 허용 하는 문서 라이브러리의 컨텍스트에서 정책 팁을 표시할 수 있습니다. 균등. 같은 정책 팁이 웹용 outlook, outlook 2013 이상, Excel 2016, PowerPoint 2016 및 Word 2016에도 나타납니다.
    
- **조직의 dlp 정책과 일치 하는 콘텐츠를 표시 하는 DLP 보고서를 확인 합니다.**
    
    조직이 DLP 정책을 어떻게 준수 하 고 있는지를 평가 하기 위해 시간에 따른 각 정책 및 규칙의 일치 횟수를 확인할 수 있습니다. DLP 정책을 사용 하 여 사용자가 정책 팁을 무시 하 고 가양성을 보고할 수 있는 경우 사용자가 보고 한 내용도 볼 수 있습니다.
...
규칙은 조직의 콘텐츠에 대 한 비즈니스 요구 사항을 적용 하는 것입니다. 정책에는 하나 이상의 규칙이 있으며 각 규칙은 조건과 동작으로 구성 됩니다. 각 규칙에 대해 조건이 충족 되 면 작업이 자동으로 수행 됩니다. 규칙은 각 정책에서 우선 순위가 가장 높은 규칙부터 시작 하 여 순차적으로 실행 됩니다.
...
DLP 정책은 중요 한 정보 **유형**으로 정의 된 중요 한 정보를 보호 하는 데 도움이 됩니다. Office 365에는 신용 카드 번호, 은행 계좌 번호, 국가 ID 번호, 여권 번호 등 사용할 수 있는 다양 한 지역의 다양 한 중요 한 정보 유형에 대 한 정의가 포함 되어 있습니다. 
  
![중요 한 정보를 사용할 수 있는 형식 목록](media/3eaa9911-bc94-44be-902f-363dbf3b07fe.png)
  
DLP 정책이 신용 카드 번호와 같은 중요 한 정보 유형을 찾을 때 16 자리 숫자는 찾지 않습니다. 각 중요 한 정보 유형은 다음과 같은 조합을 사용 하 여 정의 되 고 검색 됩니다.
...
- **콘텐츠에 대 한 액세스 제한** 사이트 콘텐츠의 경우 기본 사이트 모음 관리자, 문서 소유자 및 문서를 마지막으로 수정한 사람을 제외한 모든 사용자에 대해 문서에 대 한 사용 권한이 제한 됩니다. 이러한 사용자는 문서에서 중요 한 정보를 제거 하거나 다른 조치를 취할 수 있습니다. 문서가 규정을 준수 하는 경우 원래 사용 권한은 자동으로 복원 됩니다. 문서에 대 한 액세스를 차단 하면 문서가 사이트의 라이브러리에 특별 한 정책 팁 아이콘과 함께 표시 됩니다. 
...
알림 및 재정의를 사용 하 여 사용자에 게 DLP 정책을 교육 하 고 작업을 차단 하지 않고 계속 준수 하도록 할 수 있습니다. 예를 들어 사용자가 중요 한 정보가 포함 된 문서를 공유 하려고 할 때 DLP 정책에서 전자 메일 알림을 보내이를 통해 회사에서 정책을 재정의 하도록 허용 하는 문서 라이브러리의 컨텍스트에서 정책 팁을 표시할 수 있습니다. 균등.
...
- 가장 높은 우선 순위의 정책 팁만 표시 됩니다. 예를 들어 콘텐츠에 대 한 액세스를 차단 하는 규칙의 정책 팁은 알림을 단순히 전송 하는 규칙에서 정책 팁 위에 표시 됩니다. 이를 통해 사용자는 정책 팁의 캐스케이딩을 볼 수 없습니다.
...
인스턴스 수는 해당 규칙과 일치 하는 콘텐츠에 대해 제공 해야 하는 특정 유형의 중요 한 정보 발생 수를 의미 합니다. 예를 들어, 1 ~ 9 고유 미국 또는 영국 여권 번호가 식별 되 면 콘텐츠가 아래와 같이 표시 되는 것과 같습니다.
...
미리 구성 된 DLP 정책 템플릿을 사용 하면 HIPAA 데이터, 금융-gramm-leach-bliley-gramm-leach-bliley Act 데이터, 심지어 로캘별 개인 식별 정보 (P.I.)와 같은 특정 유형의 중요 한 정보를 쉽게 검색할 수 있습니다. 일반적인 유형의 중요 한 정보를 쉽게 찾고 보호할 수 있도록 하기 위해 Office 365에 포함 된 정책 템플릿에는 시작 하는 데 필요한 가장 일반적인 중요 한 정보 유형이 이미 포함 되어 있습니다.
...
DLP 정책을 만들 때 점차적으로 해당 영향을 평가 하 고 효과를 완벽 하 게 적용 하기 전에 테스트 효율성을 확인 하는 것을 고려해 야 합니다. 예를 들어 사용자가 작업을 완료 하기 위해 액세스 해야 하는 수천 개의 문서에 대 한 액세스를 새 DLP 정책에서 실수로 차단 하는 것을 원하지 않을 수 있습니다.
  
잠재적으로 큰 영향을 주는 DLP 정책을 만드는 경우에는 다음 순서를 따르는 것이 좋습니다.
  
1. **정책 설명이 없는 테스트 모드로 시작한** 다음 DLP 보고서 및 모든 문제 보고서를 사용 하 여 영향을 평가 합니다. DLP 보고서를 사용 하 여 일치 하는 정책의 번호, 위치, 유형 및 심각도를 볼 수 있습니다. 결과에 따라 필요에 따라 규칙을 미세 조정할 수 있습니다. 테스트 모드에서 DLP 정책은 조직에서 작업 하는 사용자의 생산성에 영향을 주지 않습니다. 
    
2. 사용자에 게 준수 정책에 대 한 정보를 제공 하 고 적용할 규칙을 준비할 수 있도록 **알림 및 정책 팁이 포함 된 테스트 모드로 이동** 합니다. 이 단계에서는 사용자에 게 가양성을 보고 하 여 규칙을 더 구체화할 수 있습니다. 
    
3. 규칙의 작업이 적용 되 고 콘텐츠가 보호 되도록 **정책에 대 한 전체 적용을 시작** 합니다. 계속 해 서 DLP 보고서와 문제 보고서 또는 알림을 모니터링 하 여 결과가 의도 한 대로 진행 되는지 확인 합니다. 
    
![테스트 모드를 사용 하 여 정책 설정에 대 한 옵션](media/49fafaac-c6cb-41de-99c4-c43c3e380c3a.png)
  
정책에서 모든 규칙에 영향을 주는 언제 든 지 DLP 정책을 해제할 수 있습니다. 그러나 규칙 편집기에서 상태를 전환 하 여 각 규칙을 개별적으로 끌 수도 있습니다.
  
![정책에서 규칙을 해제 하는 것에 대 한 옵션](media/f7b258ff-1b8b-4127-b580-83c6492f2bef.png)
  
## <a name="dlp-reports"></a>DLP 보고서

DLP 정책을 만들고 켠 후에는 해당 정책이 의도 한 대로 작동 하 고 준수 상태를 유지 하도록 하는 것이 좋습니다. dlp 보고서를 사용 하 여 시간에 따른 dlp 정책 및 규칙 일치 항목 수와 가양성 및 재정의 수를 빠르게 확인할 수 있습니다. 각 보고서에 대해 위치, 시간 프레임당 일치 하는 항목을 필터링 하 여 특정 정책, 규칙 또는 작업에 맞게 좁힐 수도 있습니다.
...
작동 방식은 다음과 같습니다. 사용자가 자신의 사이트에서 문서를 추가 하거나 변경할 때 검색 엔진은 콘텐츠를 검색 하 여 나중에 검색할 수 있도록 합니다. 이 문제가 발생 하는 동안에도 콘텐츠가 중요 한 정보를 검색 하 고 공유 되는지 확인 합니다. 발견 되는 중요 한 정보는 검색 인덱스에 안전 하 게 저장 되므로 준수 팀만 액세스할 수 있고 일반적인 사용자에 게는 액세스 하지 않아도 됩니다. 백그라운드에서 사용 하도록 설정 된 각 DLP 정책 (비동기적으로)이 검색을 자주 실행 하 여 정책과 일치 하는 모든 콘텐츠를 확인 하 고 실수로 인 한 누수 로부터 보호 하기 위해 작업을 적용 하는 경우
...
DLP는 인덱싱할 수 있는 모든 콘텐츠를 평가 합니다. 기본적으로 크롤링되는 파일 형식에 대 한 자세한 내용은 [기본 크롤링 파일 이름 확장명 및 구문 분석 된 파일 형식 (SharePoint Server 2013](https://go.microsoft.com/fwlink/p/?LinkID=627430))을 참조 하십시오.
...
Excel 2016, PowerPoint 2016 및 Word 2016에는 중요 한 정보를 식별 하 고 DLP 정책을 SharePoint Online 및 비즈니스용 OneDrive로 적용 하는 것과 동일한 기능이 포함 되어 있습니다. 이러한 Office 2016 프로그램은 해당 dlp 정책을 중앙 정책 저장소에서 직접 동기화 한 다음 사용자가 dlp 정책에 포함 된 사이트에서 연 문서를 사용할 때 dlp 정책에 대해 콘텐츠를 지속적으로 평가 합니다.
  
Office 2016의 DLP 정책 계산은 프로그램의 성능 또는 콘텐츠 작업을 수행 하는 사용자의 생산성에 영향을 주지 않도록 설계 되었습니다. 대규모 문서에서 작업 중이거나 사용자의 컴퓨터가 사용 중인 경우 정책 팁이 표시 되는 데 몇 초 정도 걸릴 수 있습니다.
