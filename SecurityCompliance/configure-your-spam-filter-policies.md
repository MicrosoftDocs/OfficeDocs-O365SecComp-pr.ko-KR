...
3. 사용자 지정 정책에만 해당,이 정책에 대 한 이름을 지정 합니다. 필요에 따라 더 자세한 설명을 지정할 수 있습니다. 기본 정책 이름을 바꿀 수 없습니다.<br/><br/>참고: 정책을 만들 때 모든 구성 설정에 표시 한 화면 됩니다. 반면, 정책, 편집 하는 경우 여러 화면을 통해 이동 해야 합니다. 설정은 두 경우 모두 동일 하지만이 절차의 나머지 부분에 정책을 편집 하는 경우 이러한 설정에 액세스 하는 방법에 설명 합니다. 
...
4. **스팸 및 대량 작업으로 전자 메일을** 페이지의 **스팸** 및 **높은 정확도 스팸**들어오는 스팸 및 대량 전자 메일에 대해 수행할 동작을 선택 합니다. **정크 메일 폴더로 메시지 이동** 기본적으로 선택 됩니다. 다른 사용 가능한 값은: 
      ...
      - **X-헤더를 추가 합니다.** 지정 된 받는 사람에 게 메시지를 전송 하지만 스팸 메시지를 식별 하기 위해 X-헤더 텍스트 메시지 헤더를 추가 합니다. 이 텍스트를 사용 하 여 식별자로, 받은 편지함 규칙을 만들 또는 다운스트림 장치를 사용 하 여 메시지를 선택적으로 있습니다. 기본 X-헤더 텍스트는 **스팸일이 메시지가 표시 됩니다**.<br/>**이 X-헤더 텍스트를 추가** 하는 입력된 상자를 사용 하 여 X-헤더 텍스트를 사용자 지정할 수 있습니다. X-헤더 텍스트를 사용자 지정 하는 경우에 다음 조건에 유의 합니다. 
      ...
      - **전자 메일 주소로 메시지 리디렉션** 메시지를 받는 사람이 아닌 지정된 전자 메일 주소로 보냅니다. **이 전자 메일 주소로 리디렉션** 입력란에 "리디렉션" 주소를 지정합니다.<br/><br/>참고: 스팸 신뢰 수준에 대 한 자세한 내용은 [Spam confidence levels](spam-confidence-levels.md)를 참조 합니다. 
...
6. **차단 목록** 페이지에서 항상 스팸으로 표시될 항목(예: 보낸 사람 또는 도메인)을 지정할 수 있습니다. 서비스는 이러한 항목과 일치하는 전자 메일에 구성된 높은 정확도 스팸 작업을 적용합니다. 
      ...
      - 도메인 차단 목록에 원치 않는 도메인을 추가 합니다. **추가**클릭![아이콘 추가](media/ITPro-EAC-AddIcon.gif), 하 고 선택 대화 상자에서 차단할 도메인을 추가 합니다. 세미콜론으로 또는 새 줄을 사용 하 여 여러 항목을 구분할 수 있습니다. **확인** **차단 목록** 페이지로 돌아가려면를 클릭 합니다.<br/><br/>**주의: 최상위 도메인을 차단 하는 경우 이기는 원하는 전자 메일 스팸으로 표시 되어야 하는 것 같습니다.** 
...
7. **허용 목록** 페이지에서 받은 편지함에 항상 배달될 항목(예: 보낸 사람 또는 도메인)을 지정할 수 있습니다. 이러한 항목의 전자 메일은 스팸 필터에 의해 처리되지 않습니다. 
      ...
      - 신뢰할 수 있는 도메인의 도메인을 허용 된 대화 상대를 추가 합니다. **추가**클릭![아이콘 추가](media/ITPro-EAC-AddIcon.gif), 하 고 선택 대화 상자에서 허용 하려는 도메인을 추가 합니다. 세미콜론으로 또는 새 줄을 사용 하 여 여러 항목을 구분할 수 있습니다. **허용 목록을** 페이지로 돌아가려면 확인을 클릭 합니다.<br/><br/>**주의: 최상위 도메인을 허용 하는 경우 되었을 하지 않으려고 전자 메일 하는 받은 편지 함으로 배달 됩니다.** 
...
9. 이 기능을 사용 하도록 설정 하려면 **다음 언어로 작성 된 전자 메일 메시지를 필터링** 확인란을 선택 합니다. 클릭 ![아이콘 추가](media/ITPro-EAC-AddIcon.gif), 다음, 선택 대화 상자에서 선택한 내용 (다중 선택이 지원)을 확인 합니다. 등의 아랍어 (월드 라는), 작성 된 메시지 필터링을 선택 하 고 **격리 메시지** 는 높은 정확도 스팸 메시지에 대해 구성 된 작업 하는 경우 아랍어로 작성 된 모든 메시지가 격리 됩니다. **확인** **국제 스팸** 창으로 돌아가려면을 클릭 합니다. 
    
10. 이 기능을 사용 하도록 설정 하려면 **다음 국가 또는 지역에서 보낸 전자 메일 메시지 필터링** 확인란을 선택 합니다. 클릭 ![아이콘 추가](media/ITPro-EAC-AddIcon.gif), 다음, 선택 대화 상자에서 선택한 내용 (다중 선택이 지원)을 확인 합니다. 등에서 오스트레일리아 (AU)를 전송 하는 모든 메시지를 필터링 하려면 선택 하 고 **격리 메시지** 는 오스트레일리아에서 전송 된 높은 정확도 스팸 메시지 후 모든 메시지에 대해 구성 된 작업 하는 경우 격리 됩니다. **확인** **국제 스팸** 창으로 돌아가려면을 클릭 합니다.<br/><br/>다국어 스팸 옵션을 선택하지 않으면 기본적으로 모든 지역에서 모든 언어로 전송된 메시지에 대해 일반 스팸 필터링이 수행됩니다. 즉, 메시지가 분석된 후 스팸 또는 높은 스팸 지수인 것으로 확인된 경우 구성된 작업이 적용됩니다. 
  
11. **고급 옵션** 페이지에서 각 고급 스팸 필터링 옵션에 대 한 **, **해제**또는 **테스트** **를 선택할 수 있습니다. 
    
12. **에서** 메시지는 적극적으로 해당 옵션와 연결 된 규칙에 따라 필터링 됩니다. 메시지는 스팸으로 표시 되어 있거나 또는 자신의 스팸 점수 증가 했습니다를 설정 하는 옵션에 따라 합니다. 
    
13. **해제** 스팸 필터 조건을 충족하는 메시지에 대해 아무런 작업도 수행되지 않습니다. 모든 옵션은 기본적으로 해제되어 있습니다. 
    
14. **테스트** 스팸 필터 조건을 만족 하는 메시지에 아무 작업도 수행 됩니다. 그러나 메시지는 의도 한 받는 사람에 게 메시지가 배달 되기 전에 X-헤더를 추가 하 여 태그가 지정 된 될 수 있습니다. 이 X-헤더 ASF 옵션 일치를 알 수 있습니다. 고급 옵션 중 하나에 대 한 **테스트** 를 지정 하는 경우에 테스트 사용이 가능한 옵션에 일치 하는 항목이 만들어질 때 적용할 다음 테스트 모드 설정을 구성할 수 있습니다. 
      ...
      - **이 주소로 숨은 참조 메시지 보내기** 이 옵션을 선택 하면 입력란에서 지정 하는 전자 메일 주소로 메시지의 숨은 참조에 포함할 보냅니다. <br/><br/>고급 스팸 필터링 옵션을 각 옵션 및 각 항목과 연결 된 X-헤더 텍스트에 대 한 설명을 포함 하는 방법에 대 한 자세한 내용은 [고급 스팸 필터링 옵션을](advanced-spam-filtering-asf-options.md)참조 하십시오. 
  
15. 사용자 지정 정책에 대 한만, **적용을** 메뉴 항목을 클릭 한 다음 사용자, 그룹 및이 정책을 적용 하는 도메인을 지정 하는 조건 기반 규칙을 만듭니다. 고유한 경우 여러 조건을 만들 수 있습니다. 
      ...
      - 도메인을 선택 하려면 **받는 사람 도메인은**를 선택 합니다. 그런 다음 후속 대화 상자에서 도메인을 추가 합니다. **확인** 주 화면으로 돌아가려면을 클릭 합니다.<br/><br/>규칙 내에서 예외를 만들 수 있습니다. 예, 특정 도메인을 제외 하 고 모든 도메인에서 메시지를 필터링 할 수 있습니다. **예외 추가**클릭 하 고 다른 조건 만들어졌는지 하는 방식에 다음과 같은 예외 조건을 만듭니다.<br/><br/>그룹에 스팸 정책을 적용 **메일 사용이 가능한 보안 그룹**에 대해서만 지원 됩니다. 
