...
## <a name="adding-a-sample-to-a-working-set"></a>작업 집합에 예제 추가

검색에서 검색 한 모든 문서를 수집 하기 전에 검색 결과의 유효성을 보다 thorougly 하는 경우 모든 항목을 추가 하는 대신 검색 결과의 무작위 샘플을 작업 집합에 추가할 수 있습니다.

작업 집합에 샘플을 추가 하려면 먼저 검색을 선택 하 고 검색 결과 플라이 아웃에서 *예제* 단추를 클릭 합니다.

그런 다음 표본 추출의 매개 변수를 선택할 수 있습니다. 다음 두 가지 옵션이 있습니다.
- 신뢰 수준 및 간격: 지정 된 통계 매개 변수를 충족 하기 위해 샘플 크기를 선택 합니다.
- 백분율: 검색에 의해 반환 된 항목 수와 선택한 매개 변수를 기준으로 샘플 크기가 결정 됩니다.

마지막으로 예제를 추가할 작업 집합을 선택 합니다. 여기에서 작업 집합에 전체 검색을 추가 하는 것과 마찬가지로 프로세스의 상태를 확인할 수 있습니다. 